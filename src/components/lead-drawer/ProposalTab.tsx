...
import {
  ...
  XCircle, Clock, Shield, Download, Trash2, Package, Search, Mail
} from 'lucide-react';
import { proposalApi, productCatalogApi } from '../../lib/api';
import { useSettings } from '../../hooks/useSettings';
import { ProposalEditor } from './ProposalEditor';
import { EmailProposalDialog } from './EmailProposalDialog';
import { WhatsAppProposalDialog } from './WhatsAppProposalDialog';
...
export function ProposalTab({ leadId, lead, userRole }: ProposalTabProps) {
  ...
  const [emailProposalTarget, setEmailProposalTarget] = useState<any>(null);
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {selectedProposal.status === 'Approved' && (
            <>
              <Button size="sm" className="bg-indigo-600 hover:bg-indigo-700" onClick={() => setEmailDialogOpen(true)}><Mail className="w-3 h-3 mr-1" />Email Proposal</Button>
              <Button size="sm" style={{ backgroundColor: '#25D366' }} className="hover:opacity-90 text-white" onClick={() => setWhatsappDialogOpen(true)}>
                <svg className="w-3 h-3 mr-1" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                WhatsApp
              </Button>
              <Button size="sm" variant="outline" onClick={() => { const to = prompt('Submitted to (client name):'); if (to) handleWorkflow('markSubmitted', { submittedTo: to }); }}><Send className="w-3 h-3 mr-1" />Mark as Submitted</Button>
            </>
          )}
          {selectedProposal.status === 'Submitted' && (
            <>
              <Button size="sm" className="bg-indigo-600 hover:bg-indigo-700" onClick={() => setEmailDialogOpen(true)}><Mail className="w-3 h-3 mr-1" />Resend Email</Button>
              <Button size="sm" style={{ backgroundColor: '#25D366' }} className="hover:opacity-90 text-white" onClick={() => setWhatsappDialogOpen(true)}>
                <svg className="w-3 h-3 mr-1" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                WhatsApp
              </Button>
                  ...
                  {products.map((p: any) => (
                    <div key={p.id} className="flex items-center justify-between bg-white p-2 rounded border text-sm cursor-pointer hover:bg-gray-50"
                      ...
                      <div>
                        ...
                        {p.is_bundle && <Badge className="ml-2 text-xs bg-purple-100 text-purple-700">Bundle</Badge>}
                      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {topLevel.length === 0 && (
                  <tr><td colSpan={7} className="px-3 py-6 text-center text-gray-500">No line items yet. Add from catalog or custom.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <EmailProposalDialog
          isOpen={emailDialogOpen && !emailProposalTarget}
        ...
        {/* WhatsApp Proposal Dialog */}
        <WhatsAppProposalDialog
          isOpen={whatsappDialogOpen && !emailProposalTarget}
  ...
  return (
    <div className="p-4 space-y-4">
            ...
            {pendingApprovals.map((p: any) => (
              <div key={p.id} className="flex items-center justify-between bg-white rounded p-2 border border-amber-100 cursor-pointer hover:bg-amber-50"
                ...
                <div>
                  ...
                  <span className="text-xs text-gray-500 ml-2">by {p.created_by_name}</span>
                </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="outline" className="text-xs">{p.proposal_type}</Badge>
                  </div>
                ...
                <div className="flex items-center gap-2">
                  {['Approved', 'Submitted'].includes(p.status) && (
                    <>
                    <Button
                      size="sm"
                      variant="outline"
                      ...
                      onClick={async (e) => {
                        e.stopPropagation();
                        const detail = await proposalApi.getById(p.id);
                        if (detail.success) {
                          setEmailProposalTarget(detail.data);
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      className="h-7 px-2 text-xs gap-1 border-green-200 hover:bg-green-50"
                      style={{ color: '#25D366' }}
                      onClick={async (e) => {
                        e.stopPropagation();
                        const detail = await proposalApi.getById(p.id);
      ...
      {/* Email Dialog for list view */}
      {emailProposalTarget && (
        <EmailProposalDialog
          isOpen={emailDialogOpen}
          onClose={() => { setEmailDialogOpen(false); setEmailProposalTarget(null); }}
          proposal={emailProposalTarget}
          lead={lead}
          onSent={async () => {
            await fetchProposals();
            setEmailProposalTarget(null);
          }}
        />
      )}

      {/* WhatsApp Dialog for list view */}
      {emailProposalTarget && (
        <WhatsAppProposalDialog
          isOpen={whatsappDialogOpen}
          onClose={() => { setWhatsappDialogOpen(false); setEmailProposalTarget(null); }}
          proposal={emailProposalTarget}
          lead={lead}
        />
      )}
      ...
      <tr className={`border-b ${item.item_type === 'Bundle' ? 'bg-purple-50/50' : ''}`}>
        <td className="px-3 py-2">
          <div className="flex items-center gap-1">
            {children.length > 0 && (
              <button onClick={() => setExpanded(!expanded)} className="text-gray-500 hover:text-gray-600">
                {expanded ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
              </button>
            )}
            <div>
              <span className="font-medium">{item.item_name}</span>
              {item.sku && <span className="text-gray-500 text-xs ml-1">({item.sku})</span>}
              {item.item_type === 'Bundle' && <Badge className="ml-1 text-xs bg-purple-100 text-purple-700">Bundle</Badge>}
              {item.item_type === 'Custom' && <Badge className="ml-1 text-xs bg-gray-100 text-gray-600">Custom</Badge>}
            </div>
          </div>
        </td>
