  ...
  return (
    <div className="bg-white border rounded-lg shadow-md overflow-y-auto text-xs" style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      <div className="p-6 border-t border-gray-200">
        ...
        {/* Annexure - Included Modules */}
        {(data.annexureItems || []).length > 0 && (
          <div className="mt-6 pt-4 border-t border-gray-200">
            ...
            {/* Group by parent */}
            {[...new Set((data.annexureItems || []).map((a: any) => a.parentName))].map(parentName => {
              const items = (data.annexureItems || []).filter((a: any) => a.parentName === parentName);
              return (
                <div key={parentName as string} className="mb-4">
                  <div className="bg-indigo-50 rounded-t-lg px-3 py-2 border border-indigo-100 border-b-0">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="font-semibold text-xs text-indigo-900">{parentName as string}</p>
                        <p className="text-xs text-indigo-600">{items.length} modules included</p>
                      </div>
                    </div>
                  </div>
                  <table className="w-full border border-indigo-100 text-xs">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="border-b border-indigo-100 px-3 py-1.5 text-right w-16 text-gray-500">Price</th>
                        <th className="border-b border-indigo-100 px-3 py-1.5 text-center w-10 text-gray-500">Qty</th>
                      </tr>
                    </thead>
                    <tbody>
                      {items.map((item: any, idx: number) => (
                        <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}>
                          ...
                          <td className="px-3 py-1.5 text-gray-500">{item.notes || '—'}</td>
                          <td className="px-3 py-1.5 text-right">{formatCurrency(item.unitPrice, undefined, { compact: false })}</td>
